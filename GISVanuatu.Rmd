...
###  Selecting hyperparameters choices
...
#Controls for single estimation
SimpleControl <- trainControl(method = "none",
                          classProbs=FALSE, 
...
Let's run a single logisitc model on the train data set

```{r}
Logit <- train(Class1 ~ . , data = train_data, 
               method = "glm",
               preProcess = c("center"),
...
pprob <- predict(Logit, train_data, type = "prob")
...
predicted_probabilities <- predict(Logit, train_data, type = "prob")
...
confusion_matrix <- confusionMatrix(predicted_classes, train_data$Class1)

# Get performance metrics
accuracy <- confusion_matrix$overall['Accuracy']
Specificty  <- confusion_matrix$byClass['Specificty']
sensitivity <- confusion_matrix$byClass['Sensitivity']
kappa <- confusion_matrix$byClass['kappa']
```
...
## Visualizing other indicators on a logit model  

Let's run a CV on the logit model (*train data set*) 


```{r}
set.seed(2512)
LogitFit<- train(Class1 ~ ., data = train_data, 
               method = "glm" , 
               preProcess = c("center"),
               trControl = ctrl)
summary(LogitFit)
