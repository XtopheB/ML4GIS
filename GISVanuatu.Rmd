...
Now, we have aligned our spatial objects (points and raster images) we can extract information from the satelite images.  At each point in our sample point data set, we will extract the information for each layer of the raster images (that are stoored in a *stack*). There are several options for extracting the information. We choose to take the mean of all values in a radius of 20 meters around each points: 

![](https://uw-madison-datascience.github.io/r-raster-vector-geospatial/images/BufferCircular.png)

This may not be nececesary, and could be refined later. 
...
Just to have a look at our train data set
