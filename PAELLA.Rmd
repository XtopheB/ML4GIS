...
# DataForModel$class <- factor(DataForModel$class, levels = names(class_labels_12), labels = class_labels_12)
# levels(DataForModel$class) <- class_labels_12
...
### Comparing bases on accuracy- Out-of-sample 
...
rf_result <- raster::predict(img,  rf_fit, na.rm =TRUE,
...
# # %>%
# #   mutate(class =  classification_fromRF)
# 
# 
# # View(subset(Table_result, is.na(class)))
# 
...
ggplot(prop_table, aes(x = reorder(Class, Proportion), y = Proportion)) +
  ...
  coord_flip()+
  theme(plot.title = element_text(hjust = 0.5))+ 
  labs(x = "", y = "", subtitle = paste ( "Random Forest model. N= ", nrow(pred_values)))  +
...
```{r}
# Export Prediction with Multinomial Logistic as TIFF
logi_result <- predict(img,
                  logi_fit,
                  filename = paste0("Data/",MyDir,"/Predictions_with_R",StudyYear,"/classification_fromlogi.tif"),  
                  overwrite = TRUE
                  )  
...
```{r}
# Create a table of class proportions
prop_table_logi <- tibble(Class = factor(Table_result_logi$class)) %>%
  count(Class) %>%
  mutate(Proportion = n / sum(n)) %>%
  arrange(desc(Proportion))
...
prop_table_logi %>%
  ...
  coord_flip()+
  theme(plot.title = element_text(hjust = 0.5))+ 
  labs(x = "", y = "", subtitle = paste ( "Logistic model. N= ", nrow(Table_result_logi)))  +
  ggtitle("Land types predicted on the whole area (logistic model) ")
...
raster_data_logi<- rast(paste0("Data/",MyDir,"/Predictions_with_R",StudyYear,"/classification_fromlogi.tif"))
...
p_logi <- ggplot() +
  ...
  labs(title = "Prediction (MNL) ",
       subtitle = paste("Out-of-sample accuray:", as.character(100*round(logi_CM$overall[1], 4)),"%" ), 
...
p_logi <- p_logi + theme(legend.position = "bottom")
...
combined_df <- raster_df_RF %>%
  ...
  rename(prediction_RF = classification_fromRF, 
         prediction_logi = classification_fromlogi)

# Create a new column indicating whether the predictions are the same or different
# the NEW class is 0!!
combined_df <- combined_df %>%
  mutate(diff = ifelse(prediction_RF == prediction_logi, "Identical", "Change"))%>%
  # mutate(diff_class = prediction_RF) %>%
  mutate(diff_class = case_when(
    prediction_RF != prediction_logi ~ "Difference",
...
StudyYear <-2020
# Read the TIFF file
raster_RF_Blanca <- rast(paste0("Data/",MyDir,"/ROI",StudyYear,"/pred_landcover_",StudyYear,"_vROIs2.tif"))
...
```{r}
# Computing the differences. We continue with a combined data frame (already logit there)
...
## Atempt to do a Sankey polot but requires to have all flows Source -> Target + intensity of flows
## This needs another file to be created (no time)

library(networkD3)
# From these flows we need to create a node data frame: it lists every entities involved in the flow
Mynodes <- data.frame(
  name=c(as.character(combined_df$prediction_RF), 
  as.character(combined_df$prediction_logi)) %>% unique()
)
 
# With networkD3, connection must be provided using id, not using real name like in the links dataframe.. So we need to reformat it.
combined_df$IDsource <- match(combined_df$prediction_RF, nodes$name)-1 
combined_df$IDtarget <- match(combined_df$prediction_logi, nodes$name)-1
 
# Make the Network
p <- sankeyNetwork(Links = combined_df, Nodes = Mynodes,
