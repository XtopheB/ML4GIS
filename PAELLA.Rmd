...
```{r,  results= TRUE }
datasummary_skim(Table_result_smote , type = "categorical",
                 title = "Prediction on the whole region",
                 notes = paste("N =", nrow(Table_result_smote))) 
```
...
#   theme_minimal() + 
#   theme(plot.title = element_text(hjust = 0.5))+ 
#   labs(x = "", y = "", subtitle = paste ( "Multinomial smotestic model. N= ", nrow(Table_result_smote)))  +
...
p_smote <- p_smote + theme(legend.position = "bottom")
...
Mynodes <- data.frame(
  ...
  as.character(combined_df$prediction_smote)) %>% unique()
)
 
# With networkD3, connection must be provided using id, not using real name like in the links dataframe.. So we need to reformat it.
combined_df$IDsource <- match(combined_df$prediction_RF, nodes$name)-1 
combined_df$IDtarget <- match(combined_df$prediction_smote, nodes$name)-1
