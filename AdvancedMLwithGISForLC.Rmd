...
While doing the prediction on thje whole area (each pixel of the raster on the ROI), we  export the results to a *.tif* file that will be used in QGIS. 

```{r}
# prediction with Random Forest model
rf_result <- predict(img,
                  rf_fit,
                  filename = "Data/GISBlanca/classification_fromRF.tif", # export to be used in QGIS
                  overwrite = TRUE
                  )  
...
logi_result <- predict(img,
                  logi_fit,
                  filename = "Data/GISBlanca/classification_fromlogi.tif",  # Export to QGIS
                  overwrite = TRUE
                  )  
...
### Comparing Random Forest model with Blanca's
