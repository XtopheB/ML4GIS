...
# DataForModel$class <- as.factor( shp$class[ match(DataForModel$MC_ID, seq(nrow(shp)) ) ])
...
ggplot() + 
  ...
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))+ 
  labs(x = "", y = "", subtitle = paste ( "N= ", nrow(Table_result)))  +
...
# Read the TIFF file
raster_data <- rast("Data/GISBlanca/classification_fromR.tif")

# Convert the raster to a data frame for ggplot2
raster_df_R <- as.data.frame(raster_data, xy = TRUE)
...
raster_df_R$classification_fromR <- factor(raster_df_R$classification_fromR, levels = names(class_labels), labels = class_labels)

# Plot using ggplot2
p_R <- ggplot() +
  geom_raster(data = raster_df_R, aes(x = x, y = y, fill = classification_fromR), alpha = 0.5 ) +
  scale_fill_manual(values = class_colors, name = "Land Cover") +
  coord_fixed() +
  theme_minimal() +
  labs(title = "Prediction (with R) ",
       x = "Longitude",
       y = "Latitude")
# Plot
p_R
...
combined_df <- raster_df_R %>%
  inner_join(raster_df_QGIS, by = c("x", "y")) %>%
  rename(prediction_R = classification_fromR, prediction_QGIS = R_RF_Classification_2020)

# Create a new column indicating whether the predictions are the same or different
combined_df <- combined_df %>%
  mutate(diff = ifelse(prediction_R == prediction_QGIS, "No Change", "Change"))

# Define the colors for the map
colors <- c("No Change" = "white", "Change" = "pink")

#Plot the map highlighting changes
p_diff <- ggplot(combined_df, aes(x = x, y = y)) +
  geom_raster(aes(fill = diff)) +
  scale_fill_manual(values = colors, name = "Differences") +
  coord_fixed() +
  theme_minimal() +
  labs(title = "Differencesin Vegetation Predictions",
...
```{r}
# Other way to represent differences- crate a new category

# define the color palette 
class_colors_C <- c(class_colors, "white")
# Define the class labels
class_labels_C <- c(class_labels, "8" = "Difference")

combined_df_C <- combined_df %>%
  mutate(full = ifelse(prediction_R ==prediction_QGIS, prediction_R, "8"))

# Convert raster values to a factor with labels
combined_df_C$full <- factor(combined_df_C$full, levels = names(class_labels_C), labels = class_labels_C)

#Plot the map highlighting changes
p_full <- ggplot(combined_df_C, aes(x = x, y = y)) +
  geom_raster(aes(fill = full)) +
 scale_fill_manual(values = class_colors_C, name = "Land Cover") +
  coord_fixed() +
  theme_minimal() +
  labs(title = "Differencesin Vegetation Predictions",
       subtitle = "R predictions vs QGIS predictions",
...
combined_plot <- (p_R |  p_QGIS) / (p_diff + p_full)  
