...
From QGIS, we import the images that coresponds to the region of interestor **ROI**. The first element is to get the images frilm the satellite. 
...
In the presence of imbalance classes, we should prefer using **kappa** instead of **accuracy** to select the best model.
...
The idea is always to resample to have a more balance sample with almost the same number of observation in each class and avoid over-predicting the dominant class. We can combine this with the coice of Kappa as the metric to select the "best" model. 
