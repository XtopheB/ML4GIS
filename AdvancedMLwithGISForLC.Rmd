...
# We need to do a bit of rearrageemtn to have the results in a way easy to plot with ggplot 
 library(tidyr)
# results_long <- as.data.frame(results)%>%
#   mutate(Resample = as.factor(Resample)) %>%
#   pivot_longer(cols = -c(Resample, accuracy), names_to = "SampleSize", values_to = "accuracy")
...
p_diff <- combined_df%>%
 ...
 scale_fill_manual(values = "black", name = "Differences") +
  coord_fixed() +
  theme_minimal() +
