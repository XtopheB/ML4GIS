...
subtitle: "Estimating a Random Forest Model on GIS files (extracted by Blanca)"
...
From QGIS, we import the images that corresponds to the region of interest or **ROI**. The first element is to get the images from the satellite. 
...
# IMPORTANT: Folders for data follows the directory structure defined during course in July
# This .Rmd  file is in "TRAINING_VANUATU_JULY2024_COMPLETE_SENT/Working_in_R"

# Importing TIFF images 
img <- brick("../Data/Results/LC_2020/stacked/stack_raster123456788A91112_MayJune2020.tif")

# Previously here: img <- brick("Data/GISBlanca/stack_raster123456788A91112_MayJune2020.tif")
```
...
Then, we have the "*ground truth*" or specific areas where we know the nature of the the land and its class. This is a limited sample and this sample is important for training our model and for validating the model trained and compute its accuracy on a validation sub sample. So that sample will define the set of points where the information is available. Here  it comes directly from QGIS as a shapefile (*.shp*).
...
shp <- read_sf("../Data/Results/LC_2020/training_input/Training_data_SHP.shp")

# Previously here shp <- read_sf("Data/GISBlanca/reference_dataset_ROIs/")
```
